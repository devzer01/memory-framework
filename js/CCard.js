function CCard(iX, iY, iType, fScalingFactor, oContainer, peek) {
    ...
    this.flipCard = function () {
        ...
        var _fnToCall = _fnFolded;

        if (_bFolded !== true) {
            playSound("card", 1, false);
            _szState = 'back';
            _fnToCall = _fnUnfolded;
        }
        ...
        createjs.Tween.get(_oSprite).to({scaleX: 0.1}, 100).call(function () {
            ...
            _oContainer.removeChild(_oSprite);
            _oSprite = createSprite(s_oGame.spriteSheet, _szState, CARD_WIDTH / 2, CARD_HEIGHT / 2, CARD_WIDTH, CARD_HEIGHT);
            _oSprite.x = _iX;
            _oSprite.y = _iY;
            _oSprite.scaleX = _oSprite.scaleY = _fScaleFactor;
            _oContainer.addChild(_oSprite);
            createjs.Tween.get(_oSprite).to({scaleX: _fScaleFactor}, 100).call(_fnToCall, _scope);
        });
    }
